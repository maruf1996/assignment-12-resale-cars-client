...
const BookingModal = ({ bookingProduct, setBookingProduct }) => {
  ...
  const handleBookingProduct = (data) => {
    ...
    axios
      .post("https://resale-cars-server.vercel.app/booking", bookingInfo, {
      ...
      <Modal.Body>
        <form onSubmit={handleSubmit(handleBookingProduct)}>
          <input
            ...
            {...register("buyerName", { required: "user name is required" })}
          />
          {errors.buyerName && (
            <p className="text-danger">{errors.buyerName?.message}</p>
          )}
          <input
            className="form-control form-control-lg mb-2"
            readOnly
            defaultValue={isUser?.email}
            {...register("buyerEmail", { required: "user email is required" })}
          />
          {errors.buyerEmail && (
            <p className="text-danger">{errors.buyerEmail?.message}</p>
          )}
          ...
          <input
            style={{ background: " #f06425", color: "white" }}
            className="btn d-block w-100 btn-lg my-3"
            type="submit"
            value="Buy"
          />
        </form>
